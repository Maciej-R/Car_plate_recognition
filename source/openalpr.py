...
def _convert_to_charp(string):
    ...
    if isinstance(string, bytes):
        return string
    raise TypeError("Expected unicode string values or ascii/bytes"\
        "values. Got: %r" % type(string))

def _convert_from_charp(charp):
    # Prepares char* output from c-functions into Python strings
    if isinstance(charp, bytes):
...
class Alpr():
    def __init__(self, country, config_file, runtime_dir):
        ...
        self._recognize_array_func.argtypes = [ctypes.c_void_p,
